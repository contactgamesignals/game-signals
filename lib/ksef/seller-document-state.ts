...
export function createSellerDocumentKsefStateAdapter(): Pick<
  ...
  "startAttempt" | "recordReferences" | "recordPreSubmitFailure" | "recordReconciliationError" | "recordAcceptance"
> {
  return {
    ...
    async recordPreSubmitFailure(input) {
      const updated = await rpc<unknown>("fail_seller_document_ksef_pre_submit", {
        p_document_id: requiredText(input.documentId, "documentId"),
        p_expected_fa3_sha256: requiredText(input.expectedSha256, "expectedSha256").toLowerCase(),
        p_error: safeErrorText(input.error),
      });
      return requireBoolean(updated, "KSeF pre-submit failure persistence");
    },
...
export async function recordAuthoritativeKsefRejection(input: {
  ...
  if (!Number.isInteger(input.statusCode) || input.statusCode < 400 || input.statusCode === 440) {
    throw new Error("An authoritative KSeF rejection requires a final non-duplicate 4xx/5xx status code.");
  }
