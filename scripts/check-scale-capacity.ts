...
const youtubeMatching = read("supabase/functions/_shared/youtube-matching.ts");
...
const migration = [
  ...
  read("supabase/migrations/20260823185200_scale_youtube_detail_queue.sql"),
  read("supabase/migrations/20260823185300_fix_youtube_detail_attempts.sql"),
].join("\n");
...
const YOUTUBE_GENERAL_DEFAULT_DAILY_BUDGET = 10_000;
const YOUTUBE_STATS_DEFAULT_DAILY_BUDGET = 10_000;
const YOUTUBE_IDS_PER_BATCH = 50;
...
const youtubeAmbiguousCandidateCapacityPerDay = YOUTUBE_GENERAL_DEFAULT_DAILY_BUDGET * YOUTUBE_IDS_PER_BATCH;
const youtubeStatsCandidateCapacityPerDay = YOUTUBE_STATS_DEFAULT_DAILY_BUDGET * YOUTUBE_IDS_PER_BATCH;
...
if (youtubeAmbiguousCandidateCapacityPerDay < 500_000) {
  throw new Error("YouTube general quota cannot validate at least 500,000 ambiguous candidates/day in 50-ID batches.");
}
if (youtubeStatsCandidateCapacityPerDay < 500_000) {
  throw new Error("YouTube stats quota cannot enrich at least 500,000 accepted candidates/day in 50-ID batches.");
}
...
const requiredYoutube = [
  ...
  "release_youtube_detail_candidates",
  "fetchVideoDetailsBatched",
  "videos:batchGetStats",
  'reserveQuota(supabase, "youtube_stats"',
  "stats_quota_limited",
];

const requiredYoutubeMatching = [
...
const requiredMigration = [
  ...
  "youtube_stats_daily_budget",
  "America/Los_Angeles",
  "youtube_detail_candidates",
  "enqueue_youtube_detail_candidates",
  "claim_youtube_detail_candidates",
  "complete_youtube_detail_candidates",
  "release_youtube_detail_candidates",
...
for (const snippet of requiredYoutubeMatching) {
  if (!youtubeMatching.includes(snippet)) throw new Error(`1000-game regression: YouTube matching is missing ${snippet}`);
}
...
const forbidden = [
  ...
  [youtube, "const allCandidateIds =", "YouTube detail lookup for every search candidate"],
  [youtube, "detailGranted < detailCallsNeeded", "YouTube search-window blocking on general detail quota"],
...
console.log(JSON.stringify({
  ...
  youtube: {
    ...
    ambiguous_detail_validation_capacity_per_day_at_default_general_quota: youtubeAmbiguousCandidateCapacityPerDay,
    best_effort_stats_enrichment_capacity_per_day_at_default_stats_quota: youtubeStatsCandidateCapacityPerDay,
    detection_blocks_on_stats_quota: false,
    ambiguous_results_are_durable: true,
  },
  twitch: {
  ...
  note: "The 100000/day YouTube Search Queries quota is the remaining external requirement for 1000 games at a 30-minute cadence. General metadata and stats quotas are now decoupled from mandatory search-window progress.",
}, null, 2));
