...
cp "$ROOT_DIR/scripts/check-seller-document-preparation.ts" "$TMP_DIR/check-seller-document-preparation.ts"
...
python3 - \
  ...
  "$TMP_DIR/check-seller-document-preparation.ts" <<'PY'
...
preparation = Path(sys.argv[4])
check = Path(sys.argv[5])
...
preparation_text = preparation_text.replace('import "server-only";\n\n', '')
preparation_text = preparation_text.replace('from "@/lib/ksef/seller-document-fa3"', 'from "./seller-document-fa3.ts"')
preparation.write_text(preparation_text, encoding="utf-8")

check_text = check.read_text(encoding="utf-8")
check_text = check_text.replace('from "@/lib/ksef/seller-document-preparation"', 'from "./seller-document-preparation.ts"')
check.write_text(check_text, encoding="utf-8")
PY

node --experimental-strip-types "$TMP_DIR/check-seller-document-preparation.ts"
