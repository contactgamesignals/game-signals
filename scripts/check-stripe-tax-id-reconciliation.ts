...
assert.match(worker, /STRIPE_LIVE_KEY_PATTERN\s*=\s*\/\^sk_live_/);
...
assert.match(worker, /if \(STRIPE_LIVE_KEY_PATTERN\.test\(secretKey\)\)[\s\S]*Deno\.env\.get\(LIVE_UNLOCK_ENV\) !== LIVE_UNLOCK_PHRASE[\s\S]*throw new Error/i);
...
assert.match(worker, /mode:\s*stripeMode\.label/);
...
console.log("Stripe Tax ID exact-snapshot enrichment, sandbox behavior, explicit LIVE lock, trigger and cron safeguards passed.");
