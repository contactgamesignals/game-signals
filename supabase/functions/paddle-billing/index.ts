...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (body.action === "status") {
      return json({
        configured: runtimeConfigured && Deno.env.get("PADDLE_BILLING_ENABLED") === "true",
        provider: "paddle",
    ...
    if (runtime.catalog.length !== 6) {
      return json({ error: "All six Paddle plan prices must be configured before checkout is enabled." }, 503);
    }
