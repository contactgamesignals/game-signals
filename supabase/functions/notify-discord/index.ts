...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const [{ data: mentionsData }, { data: channelsData }] = await Promise.all([
      supabase.from("mentions").select("id, platform, creator_name, title, url, viewer_count, view_count, signal_score, detected_at, games(title, workspace_id)").gte("detected_at", since).order("detected_at").limit(200),
      supabase.from("notification_channels").select("id, workspace_id, destination, minimum_signal_score, minimum_live_viewers").eq("type", "discord").eq("enabled", true),
    ]);

    const mentions = (mentionsData ?? []) as Mention[];
    const channels = (channelsData ?? []) as Channel[];
    if (!mentions.length || !channels.length) return json({ ok: true, delivered: 0 });

    const mentionIds = mentions.map((mention) => mention.id);
    const channelIds = channels.map((channel) => channel.id);
    const { data: existingData } = await supabase.from("delivered_notifications").select("mention_id, notification_channel_id").in("mention_id", mentionIds).in("notification_channel_id", channelIds);
    const existing = new Set((existingData ?? []).map((row) => `${row.mention_id}:${row.notification_channel_id}`));
    let delivered = 0;
