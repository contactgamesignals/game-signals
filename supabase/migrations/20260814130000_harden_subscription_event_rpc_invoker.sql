-- Harden the subscription event-ordering RPC to SECURITY INVOKER.
-- The Stripe webhook already calls this function with service_role, so it does not
-- need privilege escalation. This is a forward-only replacement; prior migrations
-- remain untouched for reproducible migration history.
